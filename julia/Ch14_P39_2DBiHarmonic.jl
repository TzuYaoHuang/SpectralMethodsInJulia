...
using Plots, LaTeXStrings, LinearAlgebra, FFTW,ToeplitzMatrices, SpecialFunctions,Statistics, ProgressLogging,Printf

# ╔═╡ 77f0aecb-7d3c-42fd-a3d0-4f6bf33325dc
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ a54f4341-399e-415e-9677-e7919bd2d039
md"Solving 2D Bi-Harmonic eigenvalue problem:

$\partial_{xxxx} u + \partial_{xxyy} u + \partial_{yyyy} u = -\lambda^2 u, \quad x,y\in[-1,1], \quad u(\pm 1) = \partial_\hat{\mathbf{n}} u(\pm 1) = 0.$

You could see Moffatt vortices!
"

# ╔═╡ 2b35efa8-acdf-4d66-8a2a-c8509b3481f7
N = 15

# ╔═╡ 16da7c61-7aba-48d4-ade7-41397101a7eb
begin
	D,x = ChebDiffMat(N)
	D2 = D^2; D2 = D2[2:end-1, 2:end-1]
	S = Diagonal([0; 1 ./(1 .-  x[2:end-1].^2); 0])
	D4 = (Diagonal(1 .-  x.^2)*D^4 - 8Diagonal(x)*D^3 - 12D^2)*S
	D4 = D4[2:end-1,2:end-1]

	Id = I(N-1)

	L = kron(Id,D4) + kron(D4, Id) + 2kron(D2,Id)*kron(Id,D2)

	E = eigen(L)
	nothing
end

# ╔═╡ 1c7d107a-e972-408c-85b2-a0d690a9b17e
begin
	λ = real.(E.values)
	sortedI = sortperm(λ)

	λ = λ[sortedI]; λ ./= λ[1]
	V = real.(E.vectors[:,sortedI])
	nothing
end

# ╔═╡ ba173141-ebcd-4ed3-9edc-052d8014eb8d
begin
    # Reverse x for plotting (Chebyshev points are in descending order)
    x_plot = reverse(x)
    ...
    for i in 1:25
        # Extract mode shape and reshape
        mode = V[:, i]
        mode_2d = reshape(mode, N-1, N-1)
        
        # Add zero boundary conditions
        mode_full = zeros(N+1, N+1)
        mode_full[2:end-1, 2:end-1] = mode_2d
        
        # Reverse both dimensions for plotting
        mode_plot = reverse(reverse(mode_full, dims=1), dims=2)
        
        # Create contour plot
        p = contourf(x_plot, x_plot, mode_plot',
                    levels=20,
                    colorbar=false,
                    axis=false,
                    grid=false,
        ...
        contour!(p, x_plot, x_plot, mode_plot',
                levels=[0],
                color=:black,
                linewidth=1.5,
                colorbar=false,
    ...
    # Combine into 5x5 layout
    plot(plots_array..., 
         layout=(5, 5), 
         size=(1200, 1200),
         plot_title="First 25 Mode Shapes - 2D Bi-Harmonic Problem",
...
manifest_format = "2.0"
project_hash = "7a5eafa8ed59c54bc542ce7fe081feba2ec3eb39"
