...
md"Solving ODE with boundary enforcement method:

$\partial_{xx} u = e^{4x},\quad \partial_x u(-1) = u(1) = 0.$

We replace one row of $\mathsf{D}$ with the Neumann boundary condition.
"

# ╔═╡ 0c98149b-10f4-4d27-9d9f-f69ab34e20be
N = 16

# ╔═╡ 117d59da-8664-488c-96f0-737043ef14d1
begin
	D,x = ChebDiffMat(N)
	D2 = D^2

    D2F = D2[2:end,2:end]
    D2F[end,:] = D[end,2:end] # Where Neumann boundary condition comes in

    f = exp.(4x[2:end]); f[end] = 0
