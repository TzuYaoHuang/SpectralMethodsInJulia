...
begin
	...
	R = Diagonal(r[2:end])
	invR = Diagonal(1 ./r[2:end])

	L = invR*D1Inner .+ D2Inner .- m^2*invR^2
	E = eigen(L)
	nothing
end

# ╔═╡ f5f36077-8974-4e98-aba8-c9865f3dbab9
md" $r^2 \partial_{rr}u + r\partial_r u + (\omega^2r^2-m^2)u = 0.$ When $r\rightarrow 0$, the governing equation effectively become $r\partial_r u  = m^2 u$. If $m = 0$ then it is homogeneous boundary conditions but soon it becomes Dirichelet one when $m\neq 0$."

# ╔═╡ 1177ba62-39d7-4d6d-99e3-e8fbe9d58d04
begin
	eVal = E.values
	ωVal = sqrt.(abs.(-eVal))
	sortedI = sortperm(ωVal)

	p = plot(layout=(3,2), size=(950, 700))
	for i∈1:6
		I = sortedI[i]
		ω = ωVal[I]
		...
		b = besselj.(m,ω*r)
		v .*= b[3]/v[3]
		scatter!(p[i], r, v, xlimit=(0,1), label="", title=L"\omega=%$(ω)")
		plot!(p[i], r, b, label="")
	end
	plot!()
end
