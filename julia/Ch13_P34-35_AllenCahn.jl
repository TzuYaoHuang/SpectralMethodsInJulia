...
md"Solving bistable Allen-Cahn equation:

$\partial_t u = \epsilon \partial_{xx}u + u - u^3.$
...
begin
	...
	@progress for iTime∈1:NTime
		if mod(iTime,10) == 0
			plot(x,v)
			plot!(xlimit=(-1,1),ylimit=(-1,2))
			frame(anim,plot!())
		end
		tNow = iTime*Δt
		v .+= Δt*(ϵ*D2*v .+ v .- v.^3)
		v[1] = fEnd(tNow); v[end] = -1 # Enforcing BCs
	end
	mp4(anim,fps=10)
end
