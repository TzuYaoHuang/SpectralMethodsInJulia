...
# ╔═╡ 939666c6-3db0-11f0-38bb-09d2e19d718c
using Polynomials, Plots, LaTeXStrings

# ╔═╡ 32da50dc-2cc8-4781-8d02-dbf3c5264d41
md"Analog to the potential flow of source and sink. Placing equal-strength source on the collocated points: equidistance and Chebyshev. The idea of Chebysheve points is that if you place several positive charges on a line and fix the two boundaries, then the stable distribution will like chebyshev points."

# ╔═╡ 9d046e67-f8c6-4a86-9431-9566602e55e8
begin
	xAna = -1.01:0.005:1.01
	N = 16

	ReList = -1.4:0.02:1.4
	ImList = -1.12:0.02:1.12
	zMat = [r+1im*i for r∈ReList, i∈ImList]

	xList = [collect(-1 .+ 2*(0:N)/N), cos.(π*(0:N)/N)]
	...
	for (ix, x)∈enumerate(xList)
		...
		plot!(pp[ix], xlimit=(-1.,1.),legend=false)

		zVal = clamp.(abs.(p.(zMat))',0,0.1)

		contour!(pc[ix], ReList, ImList, zVal, levels=10 .^(-4:0.5:0), aspect_ratio=:equal, xlimit=[-1.4,1.4], ylimit=[-1.12,1.12])
		scatter!(pc[ix], x, u, ms=2, c=:black, legend=false,colorbar=false)
	end
end

# ╔═╡ 1ca4dd03-005c-4d18-83f8-d8e200c9df61
plot(pp..., layout=(2,1))

# ╔═╡ e02beb23-4cb3-4a9c-b17c-9372bd1defba
plot(pc..., layout=(1,2))
