...
# ╔═╡ abf6bb06-3d9e-11f0-187d-afcabcf11524
using Plots, ToeplitzMatrices, LinearAlgebra, LaTeXStrings

# ╔═╡ 57fafb43-3552-4716-ab51-9c0aea30fd09
md"This example solves eigen problem for $$-\partial_{xx}u + x^2 u = \lambda u$$. The eigenvalues ($\lambda$) should be $1,3,5,7,...$."

# ╔═╡ 2fe6b645-78e8-4277-aca6-cc4698161fd7
function Diff2Matrix_Sinc(N)
	Δx = 2π/N
	diagVal = -π^2/3Δx^2 - 1/6
	vr = [ifelse(j==0, diagVal, -(-1)^j / 2sin(j*Δx/2)^2) for j∈0:N-1]
	D = Toeplitz(vr,vr)
	return D
end

# ╔═╡ cb6b1bcc-ec2a-4be2-8ca7-ba1028b03318
 begin
	 L = 20
	 for N∈6:12:108
		 Δx = 2L/N
		 x = (1:N)*Δx .- L
		 D2 = Diff2Matrix_Sinc(N)*(π/L)^2
		 eigenValues = eigvals(-D2 + diagm(x.^2))
		 display(N)
		 display(eigenValues[1:5])
		 println()
	 end
 end
