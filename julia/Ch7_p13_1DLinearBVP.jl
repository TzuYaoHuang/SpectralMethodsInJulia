...
begin
	# Solution
	b = exp.(4xInner) - uLeft*D2[2:end-1,1] - uRight*D2[2:end-1,end]
	uInner = D2Inner\b
	uSol = [uLeft, uInner..., uRight]
	...
	uuAna = ua.(xx)
	uuSol = p.(xx)
	e = norm(uuAna .- uuSol, Inf)

	# Plotting
	plot()
	plot!(xx,uuSol,lw=2,color=:black)
	scatter!(x,uSol, ms=5, color=:black)
	plot!(xlimit=[-1,1],ylimit=[-1.5,2.5], title="Maximum error: $(e)",legend=false)
end
