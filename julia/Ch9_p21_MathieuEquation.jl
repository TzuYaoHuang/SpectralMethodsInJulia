...
using Plots, LaTeXStrings, LinearAlgebra, FFTW, ToeplitzMatrices

# ╔═╡ 8ec8baff-e497-4ddb-9ec6-587bebde510a
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ 1caa76e1-68c2-4582-800b-7d07cadef61f
md"Calculate the eigenvalues of $-\partial_{xx} u + 2q\cos(2x)u = \lambda u$. Plot the evolution of the first eleven eigenvalues w.r.t $q$."
...
begin
	...
	Neigval = 11
	colors = palette(:isoluminant_cgo_70_c39_n256, Neigval)

	D2 = SincDiff2(N)

	qList = 0:0.2:15

	eigvalList = zeros(length(qList),11)

	for (iq,q)∈enumerate(qList)
		L = -D2 + Diagonal(2q*cos.(2x))
		λ = eigen(L).values; sort!(λ)
		eigvalList[iq,:] .= λ[1:Neigval]
	end

	plot(size=(300,600))
	for i∈1:Neigval
		plot!(qList, eigvalList[:,i], linecolor=colors[i])
	end
	scatter!(zeros(Neigval),(0:5).^2, color=:black)
	plot!(xlimit=(0,15), ylimit=(-24,32), xlabel=L"q", ylabel=L"\lambda",aspect_ratio=0.625, legend=false)
end
...
manifest_format = "2.0"
project_hash = "3c59e392ece5be046c40ead59f61e59a070357ad"
