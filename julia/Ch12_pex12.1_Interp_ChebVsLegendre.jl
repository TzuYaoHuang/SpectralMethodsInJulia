...
using Plots, LaTeXStrings, Polynomials, LinearAlgebra, FFTW, ToeplitzMatrices, FastGaussQuadrature

# ╔═╡ 34b6a4a7-8ff5-4de0-9991-c18641965fa4
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ 2f015ea7-bd3d-4878-b670-62c321e17b05
md"Compare the interpolation of different interpolation points distribution: Legendre and Chebyshev points."

# ╔═╡ abc03b82-b642-4702-b70a-32312cfc75b2
begin
    # --- setup ---
    δ = 0.25
    Ns = 5:5:100
    ...
    # initialize figure
    plt = plot(xlim=(-1.05, 1.05), ylim=(0, 105),
        xlabel="x", ylabel="Polynomial order N",
        title="Legendre vs. Chebyshev node distribution as N → ∞",
        legend=:topright, framestyle=:box, size=(700, 700),
        grid=false, tickfontsize=9, guidefontsize=11)
    
    for (iN, N) in enumerate(Ns)
        xL, _ = gausslegendre(N)              # Legendre-Gauss points
        xC = cos.(π * (0.5:N) ./ N)           # Chebyshev-Gauss points
    
        scatter!(xL, N .* ones(N) .+ δ,
            ms=3, markerstrokewidth=0, color=:red,
            label=(iN==1 ? "Legendre-Gauss" : ""),
        )
        scatter!(xC, N .* ones(N) .- δ,
            ms=3, markerstrokewidth=0, color=:green,
            label=(iN==1 ? "Chebyshev-Gauss" : ""),
        )
    end
...
begin
    ...
    w,xL = GaussLegendreQuad(N)

    xList = [xL, cos.(π*(0.5:N)/N)]
    lList = ["Legendre", "Chebyshev"]
    pp = [plot(), plot()]
...
# ╔═╡ e23aa037-d9c8-4245-a0e7-dcf53e863732
0.5:5
...
manifest_format = "2.0"
project_hash = "3efa7d836aab56c17bcf5a721b0d3301d1a414e2"
