...
md" $\Gamma$ function from simple complex integral along a circular countor. As metioned this integral is periodic along the integral path (It is circle!!!), so we can just use simple periodic trapezoidal integral!

$\frac{1}{\Gamma(z)} = \frac{1}{2\pi i}\int_C e^t t^{-z} \,\mathrm{d}t.$

The countour $C$ is a circle from $-\infty + 0^- i$ swapping along the origin to $-\infty + 0^+ i$. As $e^t$ decay exponentially in negative real direction. It is totally fine to use a arbitrary circlur contour to approximate it. We here choose $t = -11 + 16e^{i\theta}$, and thus $\,\mathrm{d}t = (t+11)i \,\mathrm{d}\theta.$ Thus,

$\frac{1}{\Gamma(z)} = \frac{1}{2\pi}\int_C e^t t^{-z} (t-c) \,\mathrm{d}\theta.$
"
...
# ╔═╡ 84f30ca7-8692-4e7d-81b3-8002237a4f8f
begin
	x = collect(-3.5:0.1:4)
	y = collect(-2.5:0.1:2.5)
	θ = -π .+ (0.5:N-0.5)*2π/N # avoid the branch cut
	function Γ(z)
		t = @. c + r*exp(im*θ)
		integrand = sum((τ)-> exp(τ)*τ^(-z)*(τ-c), t)/N
		return 1/integrand
	end
...
# ╔═╡ 82a7587d-2d20-4d40-8e81-1228ba28b4dd
begin
	plot()
	contourf!(x,y,clamp.(abs.(ΓList)',0,10), aspect_ratio=:equal,xlimit=(-3.5,4), ylimit=(-2.5,2.5),color=:binary)
end
...
manifest_format = "2.0"
project_hash = "0d627510aa78a063df473e2ee895c36a8cf7be77"
