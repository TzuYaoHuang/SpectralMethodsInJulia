...
md"We try to solve $\partial_{xx} u = e^{4x}$, $-1<x<1$, $u(-1) = 1$, $u(1) = 2$. The analytical solution is $u(x) = (e^{4x}-x\sinh(4)-\cosh(4)+8x+24)/16$"
...
begin
	# Solution
	# Note that the point order of Chebyshev is descending
	b = exp.(4xInner) - uRight*D2[2:end-1,1] - uLeft*D2[2:end-1,end]
	uInner = D2Inner\b
	uSol = [uRight, uInner..., uLeft]
	...
	uAna = ua.(x)
	uuAna = ua.(xx)
	e = norm(uAna .- uSol, Inf)

	# Plotting
	plot()
	plot!(xx,uuAna,lw=2,color=:black, label="Analytical")
	scatter!(x,uSol, ms=5, color=:black, label="Chebyshev")
	plot!(xlimit=[-1,1],ylimit=[-1.5,2.5], title="Maximum error: $(e)")
end
