...
md"Solving Orr-Sommerfeld Eigenvalue problem:

$\frac{1}{\mathrm{Re}}(\partial_{xxxx} u -2\partial_{xx} u + u) -2iu-i(1-x^2)(\partial_{xx}u -u) = \lambda(\partial_{xx}-u)$

$u(\pm 1) = \partial_x u (\pm 1) = 0$
"
...
begin
	p = plot(layout=(2, 2), size=(800, 800), legend=false)

	for (subplot_idx,N) in enumerate(40:20:100)
	    ...
	    Id = I(N-1)
	    A = (D4 - 2D2 + Id) / Re - 2im * Id - 1im * Diagonal(1 .- x[2:N].^2) * (D2 - Id)
	    B = D2 - Id
	    ...
	    # Plot eigenvalues
	    scatter!(p[subplot_idx], real(λ), imag(λ),
	            markersize=6,
	            markerstrokewidth=0,
	            xlims=(-0.8, 0.2),
	            ylims=(-1, 0),
	            ...
	            xlabel="Real(λ)",
	            ylabel="Imag(λ)",
	            title=@sprintf("N = %d  λₘₐₓ = %.11f", N, maximum(real(λ))))
	end
	
	plot!(p, plot_title="Orr-Sommerfeld Eigenvalues, Re = $Re",
