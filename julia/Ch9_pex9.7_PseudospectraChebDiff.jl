...
md"Pseudospectra of Chebyshev derivative operator."
...
begin
	plot(size=(650,500))
	contourf!(ReL,ImL, log10.(σmin'),levels=collect(-16:-2), color=cgrad(:roma;rev=true))
	scatter!(real.(eVal),imag.(eVal),color=:red,label="Eigenvalue")
	plot!(aspect_ratio=:equal, xlimit=(-10,10),ylimit=(-10,10),xlabel=L"\mathfrak{Re}",ylabel=L"\mathfrak{Im}", title=L"\log_{10}(\sigma_\mathrm{min}(z\mathsf{I}-\mathsf{L}))")
end
...
begin
	...
	scatter!(real.(eVal),imag.(eVal),color=:red,label="Eigenvalue")
	plot!(aspect_ratio=:equal, xlimit=(-10,10),ylimit=(-10,10),xlabel=L"\mathfrak{Re}",ylabel=L"\mathfrak{Im}", title=L"||({z\mathsf{I}-\mathsf{L}})^{-1}{||}_2")
end
