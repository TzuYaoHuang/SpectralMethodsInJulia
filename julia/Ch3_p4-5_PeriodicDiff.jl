...
md"Comparison between FFT and band-limited interpolator ($\delta$ function approach). They should be equivalent. And again they cannot handle discontinuity in derivative."
...
begin
	plot(x,u,c=:black,lw=2, label=L"f(x)")
	scatter!(x, wSinc, c=:blue, ms=8, label=L"f'_\mathrm{sinc}")
	scatter!(x, wFFT, c=:red, ms=4, label=L"f'_\mathrm{FFT}")
	plot!(xlimit=(0,2π))
end
