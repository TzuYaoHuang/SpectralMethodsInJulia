...
# ╔═╡ d8e8d674-b005-11f0-ab61-9310494d3a11
...
function plotStabilityR!(p, ρ, σ, ζm=1)
	ζ = (1+δ)ζm * exp.(1im * θ)
	plot!(p, ρ.(ζ)./σ.(ζ), color=:red)
	ζ = (1-δ)ζm * exp.(1im * θ)
	plot!(p, ρ.(ζ)./σ.(ζ), color=:green)
	ζ = ζm * exp.(1im * θ)
	plot!(p, ρ.(ζ)./σ.(ζ), color=:black)
	return p
end
...
begin
	...
	plotStabilityR!(p[i], 
					(ξ) -> ξ^3 - ξ^2, 
					(ξ) -> 23/12*ξ^2 - 16/12*ξ + 5/12)
	plotStabilityR!(p[i], 
					(ξ) -> ξ^4 - ξ^3, 
					(ξ) -> 55/24*ξ^3 - 59/24*ξ^2 + 37/24*ξ-9/24)
	plotStabilityR!(p[i], 
					(ξ) -> ξ^5 - ξ^4, 
					(ξ) -> 1901/720*ξ^4 - 2774/720*ξ^3 + 2616/720*ξ^2 - 1274/720*ξ + 251/720)
	plot!(p[i], title="Adams-Bashforth",legend=false, xlabel=L"\mathfrak{Re}(z)", ylabel=L"\mathfrak{Im}(z)", xlimit=(-2.5,1.5), ylimit=(-2,2), aspect_ratio=:equal)
	...
	plotStabilityR!(p[i], 
					(ξ) -> ξ^2 - ξ, 
					(ξ) -> 5/12*ξ^2 + 8/12*ξ - 1/12)
	plotStabilityR!(p[i], 
					(ξ) -> ξ^3 - ξ^2, 
					(ξ) -> 9/24*ξ^3 + 19/24*ξ^2 -5/24*ξ + 1/24)
	plotStabilityR!(p[i], 
					(ξ) -> ξ^4 - ξ^3, 
					(ξ) -> 251/720*ξ^4 + 646/720*ξ^3 - 264/720*ξ^2 + 106/720*ξ - 19/720)
	plot!(p[i], title="Adams-Moulton",legend=false, xlabel=L"\mathfrak{Re}(z)", ylabel=L"\mathfrak{Im}(z)", xlimit=(-7,2.5), ylimit=(-4,4), aspect_ratio=:equal)
	
	
	# Backward Differentiation formula
    i = 3
	for ii∈1:6
		ρ(ξ) = sum(j -> (1-1/ξ)^j / j, 1:ii)
		plotStabilityR!(p[i], ρ, (ξ) -> 1)
	end
	...
	for po∈1:6
		w = 0
		for (ii, ϕ)∈enumerate(θ) 
			w = w - (e(w, po)-exp(1im*ϕ*po))/e(w, po-1) # using the result from previous point so that I only need to do NR solving for once.
			zS[ii] = w
		end
		plot!(p[i], zS, color=:black)
		w = 0
		for (ii, ϕ)∈enumerate(θ) 
			w = w - (e(w, po)-(1+3δ)*exp(1im*ϕ*po))/e(w, po-1)
			zS[ii] = w
		end
		plot!(p[i], zS, color=:red)
		w = 0
		for (ii, ϕ)∈enumerate(θ) 
			w = w - (e(w, po)-(1-3δ)*exp(1im*ϕ*po))/e(w, po-1)
			zS[ii] = w
...
manifest_format = "2.0"
project_hash = "43e28e4226afda80f8188f88177de9b18e2129a5"
