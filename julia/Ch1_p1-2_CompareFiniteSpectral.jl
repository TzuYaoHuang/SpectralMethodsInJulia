...
function genDiffMat_2nd(N; Δx=2π/N)
	I = Vector{Int}(undef,2N)
	J = Vector{Int}(undef,2N)
	V = Vector{Float64}(undef,2N)

	for i ∈ 1:N
		# mod1 return 1~N instead of 0~N-1 as in mod (useful for julia indexing)
		base = 2(i-1)
		# store row by row
		I[base+1] = i; J[base+1] = mod1(i+1,N); V[base+1] =  1/2
		I[base+2] = i; J[base+2] = mod1(i-1,N); V[base+2] = -1/2
	end
	D = sparse(I, J, V, N, N)/Δx
	return D
end

# ╔═╡ 8454c279-de9d-498b-bc3f-ef869587d7d2
function genDiffMat_4th(N; Δx=2π/N)
	I = Vector{Int}(undef,4N)
	J = Vector{Int}(undef,4N)
	V = Vector{Float64}(undef,4N)

	for i ∈ 1:N
		# mod1 return 1~N instead of 0~N-1 as in mod (useful for julia indexing)
		base = 4(i-1)
		I[base+1] = i; J[base+1] = mod1(i+2,N); V[base+1] = -1/12
		I[base+2] = i; J[base+2] = mod1(i+1,N); V[base+2] =  2/3
		I[base+3] = i; J[base+3] = mod1(i-1,N); V[base+3] = -2/3
		I[base+4] = i; J[base+4] = mod1(i-2,N); V[base+4] =  1/12
	end
	D = sparse(I, J, V, N, N)/Δx
	return D
end
...
begin
	...
	for (i,N)∈enumerate(NFD)
		...
		w2 = D2*u
		w4 = D4*u
		errorL2[i] = norm(w.-w2, Inf)
		errorL4[i] = norm(w.-w4, Inf)
	end
	scatter!(plt,NFD,errorL2,label="2nd",color=:blue)
	scatter!(plt,NFD,errorL4,label="4nd",color=:red)
	plot!(plt,NFD, NFD.^(-2), linestyle=:dash, label="N^{-2}",color=:blue)
	plot!(plt,NFD, NFD.^(-4), linestyle=:dash, label="N^{-4}",color=:red)
end; nothing

# ╔═╡ 252db77a-15b3-4e0d-8082-4e7883aeaddc
function genDiffMat_periodicSpecral(N; Δx=2π/N)
...
begin
	NSD = 2:2:100
	errorSD = zeros(length(NSD))
	for (i,N)∈enumerate(NSD)
		x = -π .+ (1:N)* 2π/N
		u = f.(x)
		w = dfdx.(x)
		D = genDiffMat_periodicSpecral(N)
		ws = D*u
		errorSD[i] = norm(w.-ws, Inf)
	end
