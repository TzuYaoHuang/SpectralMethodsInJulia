...
# ╔═╡ 9aa1f768-3d6b-11f0-08a2-ddac32853870
using Plots, FFTW, ToeplitzMatrices, LinearAlgebra, LaTeXStrings

# ╔═╡ 3fb1bf56-d6f1-4b4e-9ba4-446b12b90ab4
md"The example demonstrates the accuracy of spactral differentiation is influced by the finite continuity: $C^n$ where $n<\infty$."

# ╔═╡ 769e7c8a-0010-4b16-8f22-9af4c545c88c
fList = [
	(x) -> abs(sin(x))^3, 
	(x) -> exp(-sin(x/2)^-2), 
	(x) -> 1/(1+sin(x/2)^2),
	(x) -> sin(10x)
]; labelList = [L"|\sin(x)|^3", L"e^{-\sin^{-2}(x/2)}", L"\frac{1}{1+\sin^2(x/2)}", L"\sin(10x)"]

# ╔═╡ 6890f88a-a273-4d9b-926b-a5d4478b5f42
begin
	...
	for (iF,f)∈enumerate(fList)
		plot!(x,f.(x),label=labelList[iF],lw=2)
	end
	plot!(xlimit=(0,2π),xlabel=L"x", ylabel=L"f(x)")
end

# ╔═╡ f1e2eb7a-b012-4b75-b4a3-1d04756354ce
...
# ╔═╡ b00e31b8-cf74-44c7-b2c3-f4fcb603538b
dfdxList = [
	(x) -> 3sin(x)*cos(x)*abs(sin(x)), 
	(x) -> 0.5*fList[2](x)*sin(x)/sin(x/2)^4, 
	(x) -> -sin(x/2)*cos(x/2)*fList[3](x)^2,
	(x) -> 10cos(10x)
]

# ╔═╡ 7692e99a-a832-4fbe-b3ec-1817bb009066
begin
	NList = 6:2:50
	eList = zeros(length(fList),length(NList))
	...
	for (iN,N)∈enumerate(NList)
		...
		for (iF,f)∈enumerate(fList)
			...
			w = D*u
			wAna = dfdx.(x)
			eList[iF,iN] = norm(w.-wAna, Inf)
		end
	end
end

# ╔═╡ 57a4d314-2ba3-4986-a3f0-b8b0ea559aa4
plot(); for (iF,f)∈enumerate(fList)
	plot!(NList, eList[iF,:],label=labelList[iF],lw=2)
end; plot!(yscale=:log10,legend=:bottomleft,xlabel=L"N", ylabel=L"\varepsilon_\infty")
