...
using Plots, LaTeXStrings, Polynomials, LinearAlgebra, FFTW, ToeplitzMatrices, FastGaussQuadrature, Printf

# ╔═╡ 34b6a4a7-8ff5-4de0-9991-c18641965fa4
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ 2f015ea7-bd3d-4878-b670-62c321e17b05
md"Generalized weight from points using Vandermonde matrix. Weights such that integral of polynomial is exact!"

# ╔═╡ 5a6e77af-da0a-4978-b6a1-cb08cf7e6441
begin
	println("Comparison of Clenshaw–Curtis weights:")
	println("N      max|Δw| (Vandermonde vs Clenshaw–Curtis)")
	println("------------------------------------------------")
	...
	for N in (4, 8, 16, 32, 64, 128)
	    ...
	    # Weights from Vandermonde system
	    w_vand = quad_weights(x)
	
	    # Weights from the standard Clenshaw–Curtis formula
	    w_fft, x_fft = ClenshawCurtisQuad(N)
	
	    # Sort by x (important if either function returns reversed order)
	    p = sortperm(x)
	    q = sortperm(x_fft)
	
	    diff = maximum(abs.(w_vand[p] .- w_fft[q]))
...
manifest_format = "2.0"
project_hash = "93c90a565348e0cfe953b87973c363e57960aab3"
