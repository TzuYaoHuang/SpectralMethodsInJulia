...
using Plots, LaTeXStrings, LinearAlgebra, Polynomials, Interpolations,FFTW

# ╔═╡ bf738f6a-90bf-46e4-89ba-fdbdac8bc58c
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ 3aa187de-b491-4551-878f-9b51a34df06d
md"We try to solve eigen value problem $\partial_{xx} u + \partial_{yy} u = 10\sin(8x(y-1))$, $-1<x,y<1$, $u(x = \pm 1) = 0$, $u(y = \pm 1) = 0$."
...
begin
	...
	f = 10sin.(8X.*(Y.-1))
	u = L\f;
	spy(L,c=:Spectral,title="Sparsity of Poisson matrix")
end

# ╔═╡ 915f281f-a693-491a-bcc1-6b84998b7892
begin
	uu = zeros(N+1,N+1)
	uu[2:end-1,2:end-1] .= reshape(reverse(u), N-1, N-1)

	xr = reverse(x)
	itp = linear_interpolation((xr,xr),uu)
	println("u(1/√2,1/√2) = $(itp(1/√2,1/√2))")
	
	plot()
	contourf!(xr,xr,uu',color=cgrad(:lighttemperaturemap),clim=(-0.5,0.5),lw=0)
...
manifest_format = "2.0"
project_hash = "14ce53a24b5f18d648c8ebdd4d52a407799cd68c"
