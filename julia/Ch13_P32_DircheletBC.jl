...
md"Solving ODE with particular/homogeneous methods:

$\partial_{xx} u = e^{4x},\quad u(-1) = 0, u(1) = 1.$

The particular solution is solved by Chebyshev and the particular function is $u_\text{part} = (x+1)/2.$
"
...
begin
	...
	xx = -1:.01:1
	uPlot = p.(xx)
	uExact = @. (exp(4xx) - sinh(4)*xx - cosh(4))/16 + (xx+1)/2
