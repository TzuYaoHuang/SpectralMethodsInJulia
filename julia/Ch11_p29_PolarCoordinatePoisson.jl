...
using Plots, LaTeXStrings, LinearAlgebra, FFTW, ToeplitzMatrices, SpecialFunctions, ProgressLogging; gr()
...
$\partial_{rr} + 1/r\partial-r u +1/r^2 \partial_{\theta\theta} u = f.$

$f = -r^2\sin(\theta/2)^4 + \sin(6\theta)\cos(\theta/2)^2$
...
# ╔═╡ 45e581d0-57f6-4ece-bc0b-98fc0e64eda7
begin
	# Radial Coordinate
	D,r = ChebDiffMat(Nᵣ)
	D2 = D^2
...
begin
	ρ = [r[j] for i∈1:Nₜ, j∈2:Nᵣ2+1]
	Θ = [θ[i] for i∈1:Nₜ, j∈2:Nᵣ2+1]

	f = @. -ρ*sin(Θ/2)^4 + sin(6Θ) * cos(Θ/2)^2
	f = f[:]
...
# ╔═╡ 30b47863-d175-4586-b93d-0a24f42ba77a
begin
	plot()
	heatmap!(θ, reverse(r[2:Nᵣ2+1]), reverse(reshape(u, (Nₜ,Nᵣ2))', dims=1); projection=:polar, colorbar = false)
end
