...
using Plots, LaTeXStrings, LinearAlgebra, FFTW,ToeplitzMatrices, SpecialFunctions,Statistics, ProgressLogging

# ╔═╡ 77f0aecb-7d3c-42fd-a3d0-4f6bf33325dc
include("Chebyshev.jl"); using .Chebyshev

# ╔═╡ a54f4341-399e-415e-9677-e7919bd2d039
md"Solving Laplace equation:

$\partial_{tt}u = \partial_{xx} u + \partial_{yy} u, \quad x\in[-3,3], \quad y\in[-1,1].$

Periodic in $x$ and Neumann in $y$.

Fourier discretization in $x$ chebyshev in $y$, leap frog in $t$

The boundary of $y$ is handled with matrix:

$\begin{bmatrix}
	\mathbf{\partial}_\mathrm{In} \\
	\mathbf{\partial}_\mathrm{BC}=0
\end{bmatrix} = 
\begin{bmatrix}
	\mathsf{A} & \mathsf{B} \\
	\mathsf{C} & \mathsf{D}
\end{bmatrix}\begin{bmatrix}
	...
	\mathbf{u}_\mathrm{BC}
\end{bmatrix}$

$\mathbf{u}_\mathrm{BC} = \mathsf{D}^{-1}\mathsf{C}\mathbf{u}_\mathrm{In}.$
"

# ╔═╡ 43bcf35a-b9e0-4a0b-a080-381afb11f03c
begin
    Nx = 50
    Ny = 15
    tMax = 2
...
# ╔═╡ 7547d982-75a4-4822-a612-8fcba5908c9d
begin
    Δt = 5/(Nx + Ny^2)
    NTime = round(Int, tMax/Δt)
    x = -3 .+(0:Nx-1)/Nx*2Ax
    D2x = SincDiff2(Nx)*(π/Ax)^2
	_Dy,y = ChebDiffMat(Ny)
	Dy = _Dy'
    D2y = Dy^2

	D_BcBc = Dy[[1,end],[1,end]]
	D_InBc = Dy[2:end-1,[1,end]]
	BC = D_InBc*inv(D_BcBc)
    
	XX = x * ones(Ny+1)'
	YY = ones(Nx) * y'

    u = @. exp(-8*((XX-xc)^2 + (YY-yc)^2))
    uOld = @. exp(-8*((XX-xc+Δt)^2 + (YY-yc)^2))
    uNew = copy(u)
	...
	@progress for iTime∈1:NTime
        if mod(iTime,1) == 0
			plot()
			contourf!(x,reverse(y),reverse(u,dims=2)',levels=-0.4:0.1:1.0)
			plot!(xlimit=(-Ax,Ax),ylimit=(-1,1),aspect_ratio=:equal)
			frame(anim,plot!())
		end
		uNew .= 2u .- uOld .+ Δt^2*(D2x*u .+ u*D2y)
		uNew[:,[1,end]] .= uNew[:,2:end-1]*BC
		uOld .= u; u .= uNew
    end
	mp4(anim,fps=10)
end
...
manifest_format = "2.0"
project_hash = "2a9840bd4937534a2c87916105868865a7175aa0"
