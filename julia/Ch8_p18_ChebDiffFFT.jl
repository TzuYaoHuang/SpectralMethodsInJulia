...
md"This notebook aim to calculate Chebyshev differentiation of $f(x) = e^{\sin(5x)}$ using FFT. One can cross reference p11."
...
begin
	...
	labelList = [L"| x^3 |", L"e^{1/x^2}", L"1/(1+x^2)", L"x^{10}"]
	...
	plot()
	for (iL,label)∈enumerate(labelList)
		plot!(NNList, eList[iL,:], label=label, lw=2)
	end
	plot!(xlabel=L"N", ylabel=L"\varepsilon_\infty", yscale=:log10,xlimit=[0,50],ylimit=[1e-16,1e1])
end
...
dfdx(x) = exp(x)*(sin(5x)+5cos(5x))

# ╔═╡ 2b271004-4093-4efe-9090-a37f44731e4b
begin
	xx = -1:.01:1
	uu = @. exp(xx)*sin(5xx)
